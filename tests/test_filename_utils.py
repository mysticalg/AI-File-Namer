...
class FilenameUtilsTests(unittest.TestCase):
    ...
    def test_sanitize_restructure_operations_filters_invalid_entries(self):
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            nested = folder / "sub"
            nested.mkdir(parents=True)
            (nested / "track 01.mp3").write_text("x")
            ...
            operations = [
                {"source": "messy folder", "destination": "Music/Archive/Messy Folder"},
                {"source": "messy folder/sub", "destination": "Music/Subfolders/Sub"},
                {"source": "messy folder/sub/track 01.mp3", "destination": "Music/Files"},
                {"source": "../escape", "destination": "Bad"},
            ]
